...
class FakeCentral {
  ...
  async simulateAdvertisementReceived(scanResult) {
    ...
    for (let key in scanResult.scanRecord.manufacturerData) {
      if (scanResult.scanRecord.manufacturerData.hasOwnProperty(key)) {
        manufacturerData.set(Number(key),
            Array.from(scanResult.scanRecord.manufacturerData[key]));
      }
    }
    ...
    for (let key in scanResult.scanRecord.serviceData) {
      if (scanResult.scanRecord.serviceData.hasOwnProperty(key)) {
        serviceData.set(key,
            Array.from(scanResult.scanRecord.serviceData[key]));
      }
    }
    scanResult.scanRecord.serviceData = serviceData;
